...
namespace Ginger.SolutionWindows.TreeViewItems
{
    class HTMLGingerReportsTreeItem : NewTreeViewItemBase, ITreeViewItem
    {
        ...
        private void AddNewTemplateItem()
        {
            ...
            {
                WorkSpace.Instance.UserProfile.Solution.HTMLReportsConfigurationSetList.Where(x => (x.IsSelected == true)).FirstOrDefault().HTMLReportTemplatesSeq = WorkSpace.Instance.UserProfile.Solution.HTMLReportsConfigurationSetList.Where(x => (x.IsSelected == true)).FirstOrDefault().HTMLReportTemplatesSeq - 1;
            }
        }

        private void OpenHTMLReportsFolder(object sender, RoutedEventArgs e)
        {
            HTMLReportsConfiguration _selectedHTMLReportConfiguration =  WorkSpace.Instance.UserProfile.Solution.HTMLReportsConfigurationSetList.Where(x => (x.IsSelected == true)).FirstOrDefault();
