    ...
    public class BusinessFlowsFolderTreeItem : NewTreeViewItemBase, ITreeViewItem
    {
            ...
            if (dlg.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
                try
                {
                    ...
                    for (int i = 0; i < importedBF.TargetApplications.Count; i++)
                        if ( WorkSpace.Instance.UserProfile.Solution.ApplicationPlatforms.Where(x => x.AppName == importedBF.TargetApplications[i].Name).FirstOrDefault() == null)
                        {
                    ...
                    if (importedBF.TargetApplications.Count == 0)
                    {
                        ...
                        ta.AppName =  WorkSpace.Instance.UserProfile.Solution.ApplicationPlatforms[0].AppName;
        ...
        public override void AddTreeItem()
        {
            ...
            if (GingerCore.General.GetInputWithValidation("Add " + GingerDicser.GetTermResValue(eTermResKey.BusinessFlow), GingerDicser.GetTermResValue(eTermResKey.BusinessFlow) + " Name:", ref BizFlowName))
            {
                ...
                if (WorkSpace.Instance.UserProfile.Solution.ApplicationPlatforms.Count != 1)
                {
                ...
                {
                    BizFlow.TargetApplications.Add(new TargetApplication() { AppName = WorkSpace.Instance.UserProfile.Solution.MainApplication });
