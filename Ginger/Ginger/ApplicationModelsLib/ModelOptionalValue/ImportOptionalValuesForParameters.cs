...
namespace Ginger.ApplicationModelsLib.ModelOptionalValue
{
    public class ImportOptionalValuesForParameters
    {
        ...
        /// </summary>
        /// <returns></returns>
        public DataSet GetExcelAllSheetData(string sheetName, bool isFirstRowHeader, bool isImportEmptyColumns, bool pivoteTable, bool isWhereCondition = false)
        {
            try
            {
                ...
                if (!isImportEmptyColumns)
                {
                    foreach (DataTable dt in ds.Tables)
                    {
                        ...
                        if (isWhereCondition && !string.IsNullOrEmpty(ExcelWhereCondition))
                        {
                            dtNew = dt.Select(ExcelWhereCondition).CopyToDataTable();
                        }
