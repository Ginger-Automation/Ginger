    ...
    public partial class RunSetsExecutionsHistoryPage : Page
    {
        ...
        private async void LoadExecutionsHistoryData()
        {
            ...
            await Task.Run(() => {
                ...
                {                   
                    mRunSetExecsRootFolder = ExecutionLogger.GetLoggerDirectory( WorkSpace.Instance.UserProfile.Solution.ExecutionLoggerConfigurationSetList.Where(x => (x.IsSelected == true)).FirstOrDefault().ExecutionLoggerConfigurationExecResultsFolder);
        ...
        private void GetExecutionResultsFolder(object sender, RoutedEventArgs e)
        {
            if ( WorkSpace.Instance.UserProfile.Solution != null &&  WorkSpace.Instance.UserProfile.Solution.ExecutionLoggerConfigurationSetList != null &&  WorkSpace.Instance.UserProfile.Solution.ExecutionLoggerConfigurationSetList.Count > 0)
            {
                Process.Start(Ginger.Run.ExecutionLogger.GetLoggerDirectory( WorkSpace.Instance.UserProfile.Solution.ExecutionLoggerConfigurationSetList[0].ExecutionLoggerConfigurationExecResultsFolder));
            }
            else
                return;
        }
