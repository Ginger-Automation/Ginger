...
{
    /// <summary>
    /// Interaction logic for QtestImportReviewPage.xaml
    /// </summary>
    public partial class QtestImportReviewPage : Page
    {
        GenericWindow _pageGenericWin = null;
        private RallyTestPlan mTestPlan = null;
        private string mImportDestinationPath = string.Empty;

        public QtestImportReviewPage(RallyTestPlan testPlan = null, string importDestinationPath = "")
        {
            InitializeComponent();
            mTestPlan = testPlan;
        ...
        private void SetGridView()
        {
            ...
            grdQtestTestPlaneImportReview.SetAllColumnsDefaultView(view);
            grdQtestTestPlaneImportReview.InitViewItems();

            grdQtestTestPlaneImportReview.btnRefresh.AddHandler(Button.ClickEvent, new RoutedEventHandler(RefreshGrid));
        }

        private void SetGridData()
        {
            grdQtestTestPlaneImportReview.DataSourceList = mTestPlan.TestCases;
        }
        ...
        {
            ObservableList<Object> qTest_TestCycles_List = new ObservableList<Object>();
            qTest_TestCycles_List.Add(mTestPlan);

            if (ALMIntegration.Instance.ImportSelectedTestSets(mImportDestinationPath, qTest_TestCycles_List))
                {
