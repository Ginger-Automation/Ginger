    ...
    class RQMRepository : ALMRepository
    {
        ...
        public override bool ShowImportReviewPage(string importDestinationFolderPath, object selectedTestPlan = null)
        {
            if (importDestinationFolderPath == "")
                importDestinationFolderPath =  WorkSpace.Instance.UserProfile.Solution.BusinessFlowsMainFolder;

            // get activities groups
            RQMImportReviewPage win = new RQMImportReviewPage(RQMConnect.Instance.GetRQMTestPlanFullData(WorkSpace.Instance.UserProfile.Solution.ALMServerURL, WorkSpace.Instance.UserProfile.ALMUserName, WorkSpace.Instance.UserProfile.ALMPassword, WorkSpace.Instance.UserProfile.Solution.ALMProjectKey, (RQMTestPlan)selectedTestPlan), importDestinationFolderPath);
            win.ShowAsWindow();
            ...
            if (testPlanList != null)
            {
                foreach (RQMTestPlan testPlan in testPlanList)
                {
                    ...
                    try
                    {
                        ...
                        if ( WorkSpace.Instance.UserProfile.Solution.MainApplication != null)
                        {
                            ...
                            foreach (Activity activ in tsBusFlow.Activities)
                                if (string.IsNullOrEmpty(activ.TargetApplication) == false)
                                    ...
                                    {
                                        ApplicationPlatform appAgent =  WorkSpace.Instance.UserProfile.Solution.ApplicationPlatforms.Where(x => x.AppName == activ.TargetApplication).FirstOrDefault();
                                        if (appAgent != null)
                            ...
                            if (tsBusFlow.TargetApplications.Count == 0)
                                tsBusFlow.TargetApplications.Add(new TargetApplication() { AppName =  WorkSpace.Instance.UserProfile.Solution.MainApplication });
                            foreach (Activity activ in tsBusFlow.Activities)
        ...
        {
            foreach (RQMTestPlan testPlan in RQMConnect.Instance.GetRQMTestPlansByProject( WorkSpace.Instance.UserProfile.Solution.ALMServerURL,  WorkSpace.Instance.UserProfile.ALMUserName,  WorkSpace.Instance.UserProfile.ALMPassword,  WorkSpace.Instance.UserProfile.Solution.ALMProject, System.IO.Path.Combine( WorkSpace.Instance.UserProfile.Solution.Folder, @"Documents\ALM\RQM_Configs")).OrderByDescending(item => item.CreationDate))
            {
                if (testPlan.RQMID == ExportToRQM.GetExportedIDString(businessFlow.ExternalID, "RQMID"))
                {
                    RQMTestPlan currentRQMTestPlan = RQMConnect.Instance.GetRQMTestPlanFullData( WorkSpace.Instance.UserProfile.Solution.ALMServerURL,  WorkSpace.Instance.UserProfile.ALMUserName,  WorkSpace.Instance.UserProfile.ALMPassword,  WorkSpace.Instance.UserProfile.Solution.ALMProject, (RQMTestPlan)testPlan);
        ...
        public override bool ExportBusinessFlowToALM(BusinessFlow businessFlow, bool performSaveAfterExport = false, ALMIntegration.eALMConnectType almConectStyle = ALMIntegration.eALMConnectType.Manual, string testPlanUploadPath = null, string testLabUploadPath = null)
        {
            ...
            if ( WorkSpace.Instance.UserProfile.Solution.ExternalItemsFields.Where(x => x.ItemType == "TestCase").ToList().Count == 0)
            {
            ...
            exportRes = ((RQMCore)ALMIntegration.Instance.AlmCore).ExportBusinessFlowToRQM(businessFlow,  WorkSpace.Instance.UserProfile.Solution.ExternalItemsFields, ref res);
        ...
        public override bool LoadALMConfigurations()
        {
            ...
            if (dlg.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
                ...
                string folderPath = Path.Combine( WorkSpace.Instance.UserProfile.Solution.Folder, "Configurations");
