...
namespace Ginger.Reports.GingerExecutionReport
{
    public class GingerExecutionReport
    {
        ...
        public void CreateRunSetLevelReport(ReportInfo RI)
        {
            ...
            ReportHTML = ReportHTML.Replace("{ReportCreated}", "Created By Ginger Version : " + Amdocs.Ginger.Common.GeneralLib.ApplicationInfo.ApplicationUIversion.ToString() + " | Used Report Template : '" + currentTemplate.Name + "' | Report Creation Time : " + DateTime.Now.ToString());
                                    ...
                                    foreach (ActionReport act in ac.ActionReports.Where(x => x.Status == Amdocs.Ginger.CoreNET.Execution.eRunStatus.Failed.ToString()).OrderBy(x => x.Seq))
                                    {
                                        ...
                                        foreach (HTMLReportConfigFieldToSelect selectedField_internal in currentTemplate.ActionFieldsToSelect.Where(x => (x.FieldType == Ginger.Reports.FieldsType.Field.ToString())))
                                        {
                                            ...
                                            if (selectedField_internal.FieldKey == ActionReport.Fields.Error)
                                            {
                                                ...
                                                fieldsValuesHTMLTableCells.Append("<td style='padding: 10px; border: 1px solid #dddddd; color:red;white-space:pre-wrap;white-space:-moz-pre-wrap;white-space:-pre-wrap;white-space:-o-pre-wrap;word-break: break-word;min-width:200px;'>" + ExtensionMethods.OverrideHTMLRelatedCharacters(act.GetType().GetProperty(selectedField_internal.FieldKey.ToString()).GetValue(act).ToString()) + "</td>");
                                            }
