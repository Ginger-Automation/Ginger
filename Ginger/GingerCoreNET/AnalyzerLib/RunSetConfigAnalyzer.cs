                    ...
                    foreach (ApplicationAgent AA in GR.ApplicationAgents)
                    {
                        ...
                        else
                        {
                            if (!AA.Agent.SupportVirtualAgent())
                            {
                                ...
                                AGR.Details = string.Format("The '{0}' Runner '{1}' " + GingerDicser.GetTermResValue(eTermResKey.TargetApplication) + "is mapped to the '{2}' Agent which is already configured on another Runner", GR.Name, AA.AppName, AA.AgentName);
