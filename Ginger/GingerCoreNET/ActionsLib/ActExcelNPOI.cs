...
{
    public class ActExcelNPOI : ActWithoutDriver, IExcelAction
    {
        ...
        public void ReadCellData()
        {
            ...
            sql = "Select * from [" + SheetName + where + "]"; //why
        ...
        public void WriteData()
        {
            ...
            try
            {
                ...
                if (dtFiltered.Rows.Count == 1 && SelectAllRows == false)
                {
                    DataRow r = dt.Rows[0];
                    ...
                    foreach (string vc in varColMaps)
                    {
                        ...
                        dtFiltered.DefaultView.RowFilter = GetInputParamCalculatedValue("PrimaryKeyColumn") + "=" + rowKey;
                        setDataUsedList.Add(new Tuple<string, object>(ColName, txt));
                    }
                    setDataUsedList.ForEach(x =>
                        sheet.GetRow(dt.Rows.IndexOf(dt.Select(SelectRowsWhere).First()) + 1).GetCell(dt.Columns[x.Item1].Ordinal).SetCellValue((string)x.Item2));
                    if (setDataUsedList.Count > 0)
                    {
                        using (FileStream fs = new FileStream(fileName, FileMode.Create))
                        {
                ...
                else if (dtFiltered.Rows.Count > 0 && SelectAllRows == true)
                {
                    updateSQL = @"UPDATE [" + GetInputParamCalculatedValue("SheetName") + "$] SET ";
                    foreach (string vc in varColMaps)
                    {
