...
namespace GingerCore.NoSqlBase
{
    ...
    {
        Cassandra.Cluster cluster = null;
        Couchbase.Cluster clusterCB = null;
        ...
        public bool Connect()
        {
            try
            {
                ...
                //    PoolConfiguration = new PoolConfiguration { MinSize = 10, MaxSize = 10 }
                //}, authenticator);
                //IBucket test= ClusterHelper.GetBucket("com.amdocs.digital.ms.shoppingcart.shoppingcart");
                //var query = (from a in context.Query<AirLine>()
                //             where a.Country == "United Kingdom"
                //             select a).
                //             Take(10);

                //IClusterManager clusterManager = clusterCB.CreateManager("Administrator", "Administrator");
                //IClusterCredentials authenticator=null;
                //authenticator.ClusterUsername = "Administrator";
                //authenticator.ClusterPassword = "Administrator";
                //BucketConfig test = null;
                //IResult<IList<BucketConfig>> bc = clusterManager.ListBuckets();
                //foreach (BucketConfig bucketConfig in bc.Value)
                //{
                //    if (bucketConfig.Name == "com.amdocs.digital.ms.shoppingcart.shoppingcart")
                //    {
                //        test = bucketConfig;
                //        break;
                //    }
                ...
                //        cluster = Cassandra.Cluster.Builder().AddContactPoint(HostPort[0]).WithPort(Int32.Parse(HostPort[1])).Build();
                //    else
                //        cluster = Cassandra.Cluster.Builder().WithCredentials(Db.User.ToString(), Db.Pass.ToString()).AddContactPoint(HostPort[0]).WithPort(Int32.Parse(HostPort[1])).Build();
                //}
                //else
                //{
                //    cluster = Cassandra.Cluster.Builder().AddContactPoint(Db.TNSCalculated).Build();
                //}
